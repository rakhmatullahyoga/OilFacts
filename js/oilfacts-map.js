...
// get data from csv
d3.csv("./data/Total_Oil_Supply.csv", function(err, data_map) {
	// set variable data1
	data_map1 = data_map;
	...
	// set colour
	quantize.domain([d3.min(data_map, function(d){
		return (+d[MAP_YEAR]) }),
    d3.max(data_map, function(d){
		return (+d[MAP_YEAR]) })]);
...
function redraw(nTahun){
	...
	data_map1.forEach(function(d) {
		valueHash[d[MAP_COUNTRY]] = +d[nTahun];
	});
	
	// set colour
	quantize.domain([d3.min(data_map1, function(d){
		return (+d[nTahun]) }),
    d3.max(data_map1, function(d){
		return (+d[nTahun]) })]);
